...
class SpecialApprovedPagesQueryPage extends QueryPage {
	...
	function getPageHeader() {
		...
		if ( $this->mMode == 'invalid' )
			$header .= Xml::tags(
				'p', array( 'style' => 'font-style:italic;' ), wfMessage( 'approvedrevs-invalid-description' )->parse() );
	...
	function getQueryInfo() {
		...
		#
			...
			list( $ns, $cat, $pg ) = ApprovedRevs::getApprovabilityStringsForDB();
			$conds = "";
			$conds .= ( $ns === '' )  ? '' : "(p.page_namespace NOT IN ($ns)) AND ";
			$conds .= ( $cat === '' ) ? '' : "(p.page_id NOT IN (SELECT DISTINCT cl_from FROM categorylinks WHERE cl_to IN ($cat))) AND ";
